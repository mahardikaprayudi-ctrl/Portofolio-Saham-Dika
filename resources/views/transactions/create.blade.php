<x-app-layout>
    ...
    <div class="bg-paper py-10">
        <div class="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
            ...
            <form method="POST" action="{{ route('transactions.store') }}" class="space-y-6 border-t border-[#DAD6C9] pt-8">
                @csrf
                ...
                <div class="grid gap-2 sm:grid-cols-[180px_1fr] sm:items-start">
                    ...
                    <input type="number" name="lot" value="{{ old('lot') }}" class="rounded border-[#CFC8B8] bg-[#FBFAF6] px-3 py-2 font-mono text-sm text-ink placeholder:font-sans placeholder:text-[#9A9485] focus:border-ink focus:ring-ink" placeholder="Contoh: 10">
                </div>

                <div class="grid gap-2 sm:grid-cols-[180px_1fr] sm:items-start">
                    <label class="pt-2 text-sm font-semibold text-[#3D3C36]">Harga per lembar</label>
                    <input type="number" step="0.01" name="price" value="{{ old('price') }}" class="rounded border-[#CFC8B8] bg-[#FBFAF6] px-3 py-2 font-mono text-sm text-ink placeholder:font-sans placeholder:text-[#9A9485] focus:border-ink focus:ring-ink" placeholder="Contoh: 560">
                </div>

                <div class="grid gap-2 sm:grid-cols-[180px_1fr] sm:items-start">
                    <label class="pt-2 text-sm font-semibold text-[#3D3C36]">Tanggal transaksi</label>
                    <input type="date" name="transaction_date" value="{{ old('transaction_date') }}" class="rounded border-[#CFC8B8] bg-[#FBFAF6] px-3 py-2 font-mono text-sm text-ink focus:border-ink focus:ring-ink">
                </div>
                ...
                <div class="flex flex-col gap-3 border-t border-[#DAD6C9] pt-6 sm:flex-row sm:justify-end">
                    ...
                    <button type="submit" class="rounded bg-ink px-4 py-2 text-sm font-semibold text-paper transition hover:bg-[#143125]">Simpan transaksi</button>
                </div>
            </form>
        </div>
    </div>
